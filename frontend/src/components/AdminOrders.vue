...
import { collection, getDocs, doc, getDoc, updateDoc, query, orderBy, addDoc, serverTimestamp } from 'firebase/firestore'
  ...
  setup() {
    ...
    const fetchOrders = async () => {
      try {
        ...
        const ordersWithProducts = await Promise.all(querySnapshot.docs.map(async (orderDoc) => {
          const orderData = orderDoc.data()
          const itemsWithDetails = await Promise.all(orderData.items.map(async (item) => {
            try {
              const productDocRef = doc(db, 'products', item.productId)
              const productDocSnap = await getDoc(productDocRef)
              const productData = productDocSnap.exists() ? productDocSnap.data() : null
              return {
                ...item,
                productName: productData ? productData.name : 'Unknown Product',
                imageUrl: productData ? productData.imageUrl : '/placeholder.svg'
              }
            } catch (error) {
              console.error(`Error fetching product ${item.productId}:`, error)
              return {
                ...item,
                productName: 'Error: Product Not Found',
        ...
        orders.value = ordersWithProducts
      } catch (error) {
    ...
    const generateCSV = (orders) => {
      const headers = [
        'Order ID', 'Customer Name', 'Email', 'Phone', 'Date', 'Status', 'Total', 'Payment Method',
        'Delivery Method', 'Address/Pickup Location', 'Items'
      ]
      const rows = orders.map(order => [
        ...
        order.deliveryMethod === 'delivery' 
          ? `${order.shippingInfo.street}, ${order.shippingInfo.barangay}, ${order.shippingInfo.municipality}`
          : 'Calapan City, Lumangbayan',
